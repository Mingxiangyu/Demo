...
/**
 * 在进行导出的时候，需要注意命令语句的运行环境，如果已经将mysql安装路径下的bin加入到 系统的path变量中，<br>
 * 那么在导出的时候可以直接使用命令语句，否则，就需要在执行命令语句的 时候加上命令所在位置的路径，<br>
 * 即mysql安装路径想的bin下的mysqldump命令
 *
 * @author T480s
 */
public class 导出导入mysql数据2 {

  public static void main(String[] args) throws IOException {
    InputStream is = 导出导入mysql数据2.class.getClassLoader().getResourceAsStream("jdbc.properties");
    Properties properties = new Properties();
    properties.load(is);
    导出导入mysql数据2.export(properties); // 这里简单点异常我就直接往上抛
    //    导出导入mysql数据2.importSql(properties);
  }
  ...
  private static String[] getImportCommand(Properties properties) {
    ...
    // 第一步，获取登录命令语句
    String loginCommand = "mysql -u" + username + " -p" + password + " -h" + host + " -P" + port;
    // 第二步，获取切换数据库到目标数据库的命令语句
    String switchCommand = "use " + importDatabaseName;
    // 第三步，获取导入的命令语句
    String importCommand = "source " + importPath;
    // 需要返回的命令语句数组
    return new String[] {loginCommand, switchCommand, importCommand};
  }
}

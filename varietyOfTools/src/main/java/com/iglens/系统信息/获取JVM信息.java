...
public class 获取JVM信息 {
  ...
  // @Autowired private SysPerformInfoServiceI sysPerformInfoService;

  public Logger log = Logger.getLogger(获取JVM信息.class);
  ...
  public String getNetworkThroughputForWindows() {
    ...
    try {
      ...
      rxPercent =
          formatNumber(
              (Long.parseLong(result2[0]) - Long.parseLong(result1[0]))
                  / (float) (1024 * 1024 * (SLEEP_TIME / 1000))); // 上行速率(MB/s)
      txPercent =
          formatNumber(
              (Long.parseLong(result2[1]) - Long.parseLong(result1[1]))
    ...
    return JSON.toJSONStringWithDateFormat(
        jsonObject, "yyyy-MM-dd HH:mm:ss", SerializerFeature.WriteMapNullValue);
  }

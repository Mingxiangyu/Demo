...
/** @ClassName: PwdCheckUtil.java @Description: 密码校验工具类 */
public class 验证密码复杂度 {
  ...
  //  复杂（同时包含数字，字母，特殊符号）
  public static String 复杂 =
      "^^(?![a-zA-z]+$)(?!\\d+$)(?![!@#$%^&*_-]+$)(?![a-zA-z\\d]+$)(?![a-zA-z!@#$%^&*_-]+$)(?![\\d!@#$%^&*_-]+$)[a-zA-Z\\d!@#$%^&*_-]+$";

  //  中级（包含字母和数字）
  public static String 中级 = "^(?![a-zA-z]+$)(?!\\d+$)(?![!@#$%^&*]+$)[a-zA-Z\\d!@#$%^&*]+$";

  //  简单（只包含数字或字母）
  public static String 简单 = "^(?:\\d+|[a-zA-Z]+|[!@#$%^&*]+)$";
   ...
   */
  public static boolean checkLateralKeyboardSite(
      String password, int repetitions, boolean isLower) {
    ...
    for (int i = 0; i + limit_num <= n; i++) {
      ...
      for (int j = 0; j < arrLen; j++) {
        ...
        if (isLower) {
          ...
          if ((configStr.indexOf(distinguishStr) != -1)
              || (UpperStr.indexOf(distinguishStr) != -1)) {
            flag = true;
            return flag;
          }
          // 考虑逆序输入情况下 连续输入
          String revUpperStr = new StringBuffer(UpperStr).reverse().toString();
          if ((revOrderStr.indexOf(distinguishStr) != -1)
              || (revUpperStr.indexOf(distinguishStr) != -1)) {
            flag = true;
            return flag;
          }
        } else {
  ...
  /**
   * @Title: checkKeyboardSlantSite @Description: 物理键盘，斜向连接校验， 如1qaz,4rfv, !qaz,@WDC,zaq1 返回true
   *
   * @param password 字符串
  ...
  /**
   * @Title: checkSequentialChars @Description: 评估a-z,z-a这样的连续字符,
   *
   * @param password 字符串
  ...
  /**
   * @Title: checkSequentialSameChars @Description: 验证键盘上是否存在多个连续重复的字符， 如！！！！, qqqq, 1111, ====,
   * AAAA返回true
   *
   * @param password 字符串
   * @param repetitions 重复次数

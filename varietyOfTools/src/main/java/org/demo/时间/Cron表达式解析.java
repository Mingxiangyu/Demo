...
public class Cron表达式解析 {
   ...
   * 例如在Seconds域使用8/10,则意味着从第8秒到60+8-10：58秒范围内，第8秒开始触发第一次，然后每隔10秒，即18，28，38，48，58秒等分别触发一次。 * 对于
   * *：表示匹配该域的任意值。例如在Minutes域使用*, 即表示每分钟都会触发事件。 对于问号 ?：只能用在DayofMonth和DayofWeek两个域，其作用为不指定 对于
   * -：表示范围。例如在Minutes域使用5-20，表示从5分到20分钟每分钟触发一次。直接进行拼接 对于逗号 ,：表示列出枚举值。例如在Minutes域使用5,20 ，
   * 则意味着在5和20分每分钟触发一次。 对于L：表示最后，只能出现在DayofWeek和DayofMonth域。 如果出现在DayofMonth域，只能使用L，表示当月最后一天
    ...
    if (tmpCorns.length == 6 || tmpCorns.length == 7) {
      ...
      if (!months.equals("*") && !months.equals("?")) {
        ...
        if (months.contains("/")) {
          ...
          sBuffer
              .append("从")
              .append(months.split("/")[0])
              .append("号开始")
              .append(",每")
              .append(months.split("/")[1])
              .append(cronTimeArr[4]);
      ...
      if (!dayofWeek.equals("*") && !dayofWeek.equals("?")) {
        ...
        } else if (dayofWeek.contains("L")
            && dayofWeek.length() > NumberUtils.INTEGER_ONE) { // 1L-7L
      ...
      if (!minutes.equals("*")) {
        ...
        if (minutes.contains("/")) {
          ...
          sBuffer
              .append(splitMinute[0])
              .append(cronTimeArr[1])
              .append("到")
              .append(splitMinute[1])
              .append(cronTimeArr[1])
              .append("每分钟");
